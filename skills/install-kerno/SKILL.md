...
**Hand off:** After the user picks an app and intent, read MCP tool schemas only — e.g. `kerno_list_endpoints` with required **`scope`** for explore; for environment setup load **`${CLAUDE_PLUGIN_ROOT}/skills/kerno-environment-setup/SKILL.md`**; for endpoint tests load **`${CLAUDE_PLUGIN_ROOT}/skills/kerno-endpoint-test/SKILL.md`**. Do not continue from this skill.
