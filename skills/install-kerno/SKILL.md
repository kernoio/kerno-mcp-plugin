...
> **Port ≠ workspace ready.** Always run `kerno mcp -w "$WORKSPACE"` before MCP registration — never skip because some port is already listening.
...
- [ ] **Start agent + discover MCP URL** — `kerno mcp -w "$WORKSPACE"`; parse **`MCP_URL`** per **`mcp-client-config.md`**
...
Do not rely on `kerno status` in agent shells (Ink TUI fails). Use **`kerno --version`**, **`docker info`**, **`kerno mcp -w "$WORKSPACE"`**, and **`kerno_get_applications`**.
