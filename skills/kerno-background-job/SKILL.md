...
1. Launch with **`kerno_endpoint_test`** — note **`job_id`** and **`log_path`**.
...
**Hard stops:** endpoint-test plan gates park the run — check via the read plane / **`kerno_feedback_pending`**, not **`kerno_job`** alone. See **`state-and-jobs.md`**.
