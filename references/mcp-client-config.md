...
**Plugin install:** see [codex/README.md](../codex/README.md) — `codex plugin marketplace add kernoio/kerno-mcp-plugin`, then `/plugins` → install **kerno**. The plugin may ship a bundled [`.mcp.json`](../.mcp.json) template; still update the URL after `kerno mcp -w` because the port is session-specific.

**Plugin MCP policy** (optional, after install): `~/.codex/config.toml` may include `[plugins."kerno@…".mcp_servers.kerno]` — see [Codex plugin MCP docs](https://developers.openai.com/codex/plugins/build).
