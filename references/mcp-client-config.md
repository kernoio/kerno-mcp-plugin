...
**MCP template:** the plugin may ship a bundled [`mcp.json.example`](../mcp.json.example) template; still update the URL after `kerno init -w` because the port is session-specific.
