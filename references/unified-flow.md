...
3. **Choose environment** — default **`local`**. See [target-environment.md](target-environment.md). Start the stack with the repo's dev flow (`docker-compose`, dev scripts, quickstart), then **`kerno_save_config`** (`local` + `sut_url` + dependencies). Use **`remote`** when the env runs somewhere other than this machine (e.g. cloud). Prefer **greybox** (DB env blocks so DB-backed scenarios run); **black box** (HTTP-only) is the fallback when DB access isn't possible.
...
- [target-environment.md](target-environment.md) — local / remote and greybox / black-box decisions
