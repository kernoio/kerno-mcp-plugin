...
Installing the plugin does **not** start the Kerno agent. **`@install-kerno`** installs `@kerno/cli`, runs `kerno mcp -w <workspace>`, and registers the session MCP URL. Full steps: [references/mcp-client-config.md](../references/mcp-client-config.md). Bundled MCP template: [`.mcp.json`](../.mcp.json) (replace URL after each `kerno mcp -w`).
